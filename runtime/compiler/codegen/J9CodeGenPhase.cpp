...
void J9::CodeGenPhase::performPatchableJProfCodeGenPhase(TR::CodeGenerator *cg, TR::CodeGenPhase *phase)
{
    ...
    if (comp->getOptimizationPlan()->insertPatchableJProfiling() && comp->getRecompilationInfo() != NULL) {
        ...
        if (info != NULL) {
            ...
            if (!instrList->empty()) {
                TR::JProfBFPatchSites *sites = new (comp->trPersistentMemory()) TR::JProfBFPatchSites(
                    comp->trPersistentMemory(), instrList->size(), cg->getMaxLengthOfIncMemoryInstruction());
