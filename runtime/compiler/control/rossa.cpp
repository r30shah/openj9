...
const char *compilationErrorNames[] = { "compilationOK", // 0
...
extern "C" void getOutOfIdleStates(TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo *compInfo,
    const char *reason);

extern "C" IDATA launchGPU(J9VMThread *vmThread, jobject invokeObject, J9Method *method, int deviceId, I_32 gridDimX,
    I_32 gridDimY, I_32 gridDimZ, I_32 blockDimX, I_32 blockDimY, I_32 blockDimZ, void **args);
...
extern "C" IDATA j9jit_testarossa(struct J9JITConfig *jitConfig, J9VMThread *vmThread, J9Method *method,
    void *oldStartPC)
{
    return j9jit_testarossa_err(jitConfig, vmThread, method, oldStartPC, NULL);
}

extern "C" IDATA j9jit_testarossa_err(struct J9JITConfig *jitConfig, J9VMThread *vmThread, J9Method *method,
    ...
    if (plan) {
        ...
        if (compInfo->getSamplerState() == TR::CompilationInfo::SAMPLER_DEEPIDLE
            ...
            && compInfo->_intervalStats._numFirstTimeCompilationsInInterval
                >= TR::Options::_numFirstTimeCompilationsToExitIdleMode)
    ...
    } else // OOM; Very rare case
    {
        ...
        if (event._eventType == TR_MethodEvent::MethodBodyInvalidated) {
            ...
            { // scope for details
                TR::IlGeneratorMethodDetails details(method);
                result
                    = (IDATA)compInfo->compileMethod(vmThread, details, oldStartPC, async, compErrCode, &queued, &plan);
            }
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%6u Disable further compilation due to OOM while creating an optimization plan",
...
extern "C" IDATA retranslateWithPreparationForMethodRedefinition(struct J9JITConfig *jitConfig, J9VMThread *vmThread,
    J9Method *method, void *oldStartPC)
{
    return retranslateWithPreparation(jitConfig, vmThread, method, oldStartPC,
        TR_PersistentMethodInfo::RecompDueToInlinedMethodRedefinition);
}

extern "C" IDATA retranslateWithPreparation(struct J9JITConfig *jitConfig, J9VMThread *vmThread, J9Method *method,
    void *oldStartPC, UDATA reason)
{
    if (!TR::CompilationInfo::get()->asynchronousCompilation()
        && !J9::PrivateLinkage::LinkageInfo::get(oldStartPC)->recompilationAttempted()) {
        TR::Recompilation::fixUpMethodCode(oldStartPC);
    }
...
extern "C" void *old_translateMethodHandle(J9VMThread *currentThread, j9object_t methodHandle)
{
...
extern "C" void *translateMethodHandle(J9VMThread *currentThread, j9object_t methodHandle, j9object_t arg, U_32 flags)
{
...
extern "C" J9Method *getNewInstancePrototype(J9VMThread *context)
{
    ...
    J9Class *jlClass = intFunc->internalFindKnownClass(context, J9VMCONSTANTPOOL_JAVALANGCLASS,
        J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
    if (jlClass) {
        newInstanceProto = (J9Method *)intFunc->javaLookupMethod(context, jlClass,
            (J9ROMNameAndSignature *)&newInstancePrototypeNameAndSig, 0,
...
extern "C" UDATA j9jit_createNewInstanceThunk_err(struct J9JITConfig *jitConfig, J9VMThread *vmThread,
    J9Class *classNeedingThunk, TR_CompilationErrorCode *compErrCode)
{
    TR::CompilationInfo *compInfo = getCompilationInfo(jitConfig);
...
extern "C" UDATA j9jit_createNewInstanceThunk(struct J9JITConfig *jitConfig, J9VMThread *vmThread,
...
extern "C" void stopSamplingThread(J9JITConfig *jitConfig)
{
...
extern "C" void freeJITConfig(J9JITConfig *jitConfig)
{
...
//
extern "C" void jitExclusiveVMShutdownPending(J9VMThread *vmThread)
{
...
//
extern "C" U_8 *getCodeCacheWarmAlloc(void *codeCache)
{
    TR::CodeCache *cc = static_cast<TR::CodeCache *>(codeCache);
    return cc->getWarmCodeAlloc();
}

extern "C" U_8 *getCodeCacheColdAlloc(void *codeCache)
{
    TR::CodeCache *cc = static_cast<TR::CodeCache *>(codeCache);
...
extern "C" void disableJit(J9JITConfig *jitConfig)
{
    TR::CompilationInfo *compInfo = getCompilationInfo(jitConfig);
    ...
    if (compInfo && compInfo->getNumCompThreadsActive() > 0) {
        ...
        if (samplerState != TR::CompilationInfo::SAMPLER_NOT_INITIALIZED
            && // jitConfig->samplerThread && // Must have a sampler thread
            ...
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%u\tSampling thread suspended and changed frequency to %d ms", persistentInfo->getElapsedTime(),
...
extern "C" void enableJit(J9JITConfig *jitConfig)
{
    TR::CompilationInfo *compInfo = getCompilationInfo(jitConfig);
    J9JavaVM *vm = jitConfig->javaVM;
    if (compInfo && compInfo->getNumCompThreadsActive() == 0) {
        ...
        if (samplerState == TR::CompilationInfo::SAMPLER_SUSPENDED) // Nothing to do if sampler is not suspended
        {
            ...
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_SAMPLING,
                    "t=%u\tSampling thread interrupted and changed frequency to %d ms",
...
extern "C" I_32 command(J9VMThread *vmThread, const char *cmdString)
{
    TR::CompilationInfo *compInfo = TR::CompilationInfo::get();

    if (strncmp(cmdString, "beginningOfStartup", 18) == 0) {
        TR::Options::getCmdLineOptions()->setOption(
            TR_AssumeStartupPhaseUntilToldNotTo); // let the JIT know it should wait for endOfStartup hint
        if (compInfo) {
            TR::PersistentInfo *persistentInfo = compInfo->getPersistentInfo();
    ...
    if (strncmp(cmdString, "endOfStartup", 12) == 0) {
        if (TR::Options::getCmdLineOptions()->getOption(
                TR_AssumeStartupPhaseUntilToldNotTo)) // refuse to do anything if JIT option was not specified
        {
            if (compInfo) {
                TR::PersistentInfo *persistentInfo = compInfo->getPersistentInfo();
...
static IDATA internalCompileClass(J9VMThread *vmThread, J9Class *clazz)
{
    J9JavaVM *javaVM = vmThread->javaVM;
    ...
    for (uint32_t m = 0; m < clazz->romClass->romMethodCount; m++) {
        ...
        if (!(romMethod->modifiers & (J9AccNative | J9AccAbstract)) && method != newInstanceThunk
            && !TR::CompilationInfo::isCompiled(method) && !fe->isThunkArchetype(method)) {
            bool queued = false;
            TR_MethodEvent event;
            event._eventType = TR_MethodEvent::InterpreterCounterTripped;
            event._j9method = method;
...
extern "C" IDATA compileClasses(J9VMThread *vmThread, const char *pattern)
{
...
#define CLASSNAME_BUFFER_LENGTH 1000
            ...
            if (J9UTF8_LENGTH(clazzUTRF8) >= CLASSNAME_BUFFER_LENGTH) {
                classNameString
                    = (char *)j9mem_allocate_memory((J9UTF8_LENGTH(clazzUTRF8) + 1) * sizeof(char), J9MEM_CATEGORY_JIT);
                if (!classNameString) {
...
extern "C" jint onLoadInternal(J9JavaVM *javaVM, J9JITConfig *jitConfig, char *xjitCommandLine, char *xaotCommandLine,
    UDATA flagsParm, void *reserved0, I_32 xnojit)
{
    ...
    if (!(jitConfig->codeCacheList)) {
        if ((jitConfig->codeCacheList
                = javaVM->internalVMFunctions->allocateMemorySegmentList(javaVM, 3, J9MEM_CATEGORY_JIT))
            == NULL)
            return -1;
    }
    ...
    bool produceRSSReport
        = produceRSSReportDetailed || TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseRSSReport);
    ...
    if (TR::Options::_overrideCodecachetotal
        && TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCodeCache)) {
        ...
        if (freePhysicalMemoryB != OMRPORT_MEMINFO_NOT_AVAILABLE)
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "Available freePhysicalMemory=%llu MB, allocating code cache total size=%lu MB",
                freePhysicalMemoryB >> 20, jitConfig->codeCacheTotalKB >> 10);
        else
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "Allocating code cache total=%llu MB due to low available memory", jitConfig->codeCacheTotalKB >> 10);
    }
    ...
    if (jitConfig->codeCacheKB > jitConfig->codeCacheTotalKB) {
        /* Handle case where user has set codeCacheTotalKB smaller than codeCacheKB (e.g. -Xjit:codeTotal=256) by
         * setting codeCacheKB = codeCacheTotalKB */
        jitConfig->codeCacheKB = jitConfig->codeCacheTotalKB;
    }

    if (jitConfig->dataCacheKB > jitConfig->dataCacheTotalKB) {
        /* Handle case where user has set dataCacheTotalKB smaller than dataCacheKB (e.g. -Xjit:dataTotal=256) by
         * setting dataCacheKB = dataCacheTotalKB */
    ...
    if (!TR_TranslationArtifactManager::initializeGlobalArtifactManager(jitConfig->translationArtifacts,
            jitConfig->javaVM))
        return -1;
    ...
    codeCacheConfig._verboseReclamation
        = TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCodeCacheReclamation);
    ...
    codeCacheConfig._codeCacheFreeBlockRecylingEnabled
    ...
    codeCacheConfig._canChangeNumCodeCaches
        = (TR::Options::getCmdLineOptions()->getNumCodeCachesToCreateAtStartup() <= 0);
    ...
    if (TR::Options::_hwProfilerEnabled == TR_yes) {
        ...
        ((TR_JitPrivateConfig *)(jitConfig->privateConfig))->hwProfiler
            = TR::Compiler->target.cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) ? TR_PPCHWProfiler::allocate(jitConfig) : NULL;
#else
    ...
    if (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER) {
        ...
        if (jitConfig->samplingFrequency != 0) {
            ((TR_JitPrivateConfig *)(jitConfig->privateConfig))->statisticsThreadObject
                = JITServerStatisticsThread::allocate();
        ...
        compInfo->setNewlyExtendedClasses(new (PERSISTENT_NEW) PersistentUnorderedMap<TR_OpaqueClassBlock *, uint8_t>(
            PersistentUnorderedMap<TR_OpaqueClassBlock *, uint8_t>::allocator_type(
    ...
    if (!TR::Compiler->target.cpu.isI386()) {
        TR_MHJ2IThunkTable *ieThunkTable
            = new (PERSISTENT_NEW) TR_MHJ2IThunkTable(persistentMemory, "InvokeExactJ2IThunkTable");
    ...
    if (TR::Options::getCmdLineOptions()->getTransientClassRegex()
        ...
        compInfo->setTransientClassLoadersSet(new (PERSISTENT_NEW) PersistentUnorderedSet<J9ClassLoader *>(
            PersistentUnorderedSet<J9ClassLoader *>::allocator_type(TR::Compiler->persistentAllocator())));
...
static int32_t J9THREAD_PROC fetchServerCachedAOTMethods(void *entryarg)
{
    ...
    try {
        ...
        client->write(JITServer::MessageType::AOTCacheMap_request, persistentInfo->getJITServerAOTCacheName());
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Received %d methods", cachedMethods.size());
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "JITServer::StreamFailure: %s", e.what());

        JITServerHelpers::postStreamFailure(OMRPORT_FROM_J9PORT(vm->portLibrary), compInfo,
...
extern "C" int32_t aboutToBootstrap(J9JavaVM *javaVM, J9JITConfig *jitConfig)
{
    ...
    if ((persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT)
        ...
        if (persistentInfo->getJITServerAOTCacheIgnoreLocalSCC()) {
            ...
            fprintf(stderr,
                "Disabling JITServer AOT cache since AOT compilation and JITServerAOTCacheIgnoreLocalSCC are "
                "disabled\n");
    ...
    Trc_JIT_portableSharedCache_enabled_or_disabled(curThread,
        J9_ARE_ANY_BITS_SET(javaVM->extendedRuntimeFlags2, J9_EXTENDED_RUNTIME2_ENABLE_PORTABLE_SHARED_CACHE) ? 1 : 0);
        ...
        if (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT) {
            if (JITServerHelpers::isServerAvailable()) {
                ...
                IDATA result = javaVM->internalVMFunctions->createThreadWithCategory(
                    (omrthread_t *)&(jitConfig->serverAOTQueryThread), javaVM->defaultOSStackSize,
                    J9THREAD_PRIORITY_NORMAL, 0, &fetchServerCachedAOTMethods, (void *)jitConfig,
                    J9THREAD_CATEGORY_SYSTEM_JIT_THREAD);

                if (result != J9THREAD_SUCCESS) {
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Query thread not created");
                }
            }
        }
    }
...
#ifdef ENABLE_GPU
extern I_32 jitCallGPU(J9VMThread *vmThread, J9Method *method, char *programSource, jobject invokeObject, int deviceId,
    I_32 gridDimX, I_32 gridDimY, I_32 gridDimZ, I_32 blockDimX, I_32 blockDimY, I_32 blockDimZ, I_32 argCount,
    void **args);

IDATA launchGPU(J9VMThread *vmThread, jobject invokeObject, J9Method *method, int deviceId, I_32 gridDimX,
    I_32 gridDimY, I_32 gridDimZ, I_32 blockDimX, I_32 blockDimY, I_32 blockDimZ, void **args)
{
    TR::CompilationInfo *compInfo = getCompilationInfo(jitConfig);
    bool queued = false;
    ...
    if (plan) {
        ...
        { // scope for details
            ...
            if (result == 0) {
                ...
                if (GPUCompileCPUCode) {
                    typedef I_32 (*gpuCodePtr)(J9VMThread *vmThread, J9Method *method, char *programSource,
                        jobject invokeObject, int deviceId, I_32 gridDimX, I_32 gridDimY, I_32 gridDimZ, I_32 blockDimX,
                        I_32 blockDimY, I_32 blockDimZ, I_32 argCount, void **args);

                    printf("In    launchGPU: %p %p %p %p %d %d %d %d %d %d %d %d %p\n", vmThread, method,
                        plan->getGPUIR(), invokeObject, deviceId, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY,
                        blockDimZ, argCount, args);

                    gpuCodePtr gpuCode = (gpuCodePtr)TR::CompilationInfo::getJ9MethodExtra(method);

                    if ((int64_t)gpuCode != 0x1)
                        result = gpuCode(vmThread, method, plan->getGPUIR(), invokeObject, deviceId, gridDimX, gridDimY,
                            gridDimZ, blockDimX, blockDimY, blockDimZ, argCount, args);
                } else {
                    result = jitCallGPU(vmThread, method, plan->getGPUIR(), invokeObject, deviceId, gridDimX, gridDimY,
                        gridDimZ, blockDimX, blockDimY, blockDimZ, argCount, args);
                }
...
static UDATA forEachIterator(J9VMThread *vmThread, J9StackWalkState *walkState)
{
    ...
    if (!(romMethod->modifiers & J9AccNative) && // Never change the extra field of a native method
        ...
        !(TR::CompilationInfo::getJ9MethodVMExtra(j9method) == J9_JIT_QUEUED_FOR_COMPILATION)
        && // No change needed if already queued
