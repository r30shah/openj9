...
TR::S390InterfaceCallReadOnlySnippet::emitSnippetBody()
   ...
   TR::SymbolReference *interfaceCallReadOnlySymRef = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390interfaceCallHelperReadOnlyMultiSlots,
                                                                                                               false, false, false);
...
TR_Debug::print(TR::FILE *pOutFile, TR::S390InterfaceCallReadOnlySnippet *snippet)
   {
   uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
   TR::SymbolReference *resolveVirtualDispatchReadOnlySymRef = _cg->getSymRef(TR_S390interfaceCallHelperReadOnlyMultiSlots);
