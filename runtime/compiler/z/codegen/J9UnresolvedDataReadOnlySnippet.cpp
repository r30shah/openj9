...
J9::Z::UnresolvedDataReadOnlySnippet::emitSnippetBody()
   ...
   *reinterpret_cast<int32_t*>(cursor) = getCPIndex();
   ...
   TR_ASSERT_FATAL(cg()->canUseRelativeLongInstructions(startResolutionSeqLabelAddr), "startResolveSequenceLabel [%p] is outside relative immediate range", startResolutionSeqLabelAddr);*reinterpret_cast<int32_t*>(cursor) = static_cast<int32_t>(startResolutionSeqLabelAddr - (intptr_t)helperCallRA);
