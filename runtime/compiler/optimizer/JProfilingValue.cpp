...
void TR_JProfilingValue::cleanUpAndAddProfilingCandidates(TR::list<TR::TreeTop *, TR::Region &> &valueProfilingPlaceHolderCalls)
{
...
void TR_JProfilingValue::lowerCalls(TR::list<TR::TreeTop *, TR::Region &> &valueProfilingPlaceHolderCalls)
{
    ...
    for (auto iter = valueProfilingPlaceHolderCalls.begin(); iter != valueProfilingPlaceHolderCalls.end(); ++iter) {
        ...
        if (!stopProfiling) {
            ...
            TR::SymbolReferenceTable::jProfileValueWithNullCHKSymbol);
            dumpOptDetails(comp(), "%s Replacing profiling placeholder n%dn with value profiling trees, needNullTest = %s\n",
    ...
    TR_AbstractHashTableProfilerInfo *table, TR::Node *bciNode, bool addNullCheck, bool extendBlocks, bool trace)
{
    ...
    TR::Node *mainlineReturnEntryGlRegDeps = origBlockExitGlRegDeps != NULL ? mainlineReturn->getEntry()->getNode()->getFirstChild() : NULL;
    ...
    if (addNullCheck) {
         ...
         */
        TR::Node *nullTest
            = TR::Node::createif(TR::ifacmpeq, profilingValue, TR::Node::aconst(bciNode, 0), mergeBlockForProfilingCode->getEntry());
    ...
    if (mergeBlockForProfilingCode->getEntry()->getNode()->getNumChildren() == 1) {
        TR::Node *helperCallBlockGlRegDeps = mergeBlockForProfilingCode->getEntry()->getNode()->getFirstChild()->duplicateTree();
