...
TR_JProfilingValue::performOnNode(TR::Node *node, TR::TreeTop *cursor, TR_BitVector *alreadyProfiledValues, TR::NodeChecklist *checklist, TR::list<TR::TreeTop *> &valueProfilingPlaceHolderCalls)
   ...
   if (profiler != NULL)
      ...
      // as first child of the node we are profiling. In case of the
      // instanceof/checkcast we at this point we do not have VFT load generated
      // as for instanceof/checkcast, the profiling trees are inserted before it
      // and in case we can not lower all profiling calls that we have in the
      // method, we would need to clean-up the placeholder calls. In such case
      // it is possible that we end up with the VFT load of the object will be
      // left anchored in its original position unguarded. To prevent this
      // issue, generating VFTLoad for such case is delayed till we are
      // inserting trees for profiing that value. Given that we do not profile
      // objects currently, it is OK to set object child of instanceof/checkcast
      // in the list to keep track of already profiled values, if in future this
      // change, we need to ensure that we use correct child to keep track of
      // values being profiled.
...
TR_JProfilingValue::addProfilingTrees(
   ...
   // If adding profiling trees while lowering trees, it is expected to have
   // node that is valueProfilingPlaceHolder call. If addProfilingTrees is
   // called post trees lowering, it is called directly if the node to profile.
   ...
   // will be inserted inlined. In case we are adding trees in warm/cold
   // optimizations which will be light-weight profiling compilations where if
   // there is no advantage in recompiling method, simply branches in mainline
   // can be be NOPED. In case of inserting trees in mainline, we will use the
   // original node as the profilingValue node, otherwise if trees are inserted
   // in cold block - we need to find the value node in the glRegDeps or store it
   // in temp slot for profiling code.
   ...
   // Register was not avaialble to store the value, must be stored in tempslot by block splitter.
   ...
   if (childRegDepIndexForProfilingValue == -1)
      ...
      TR_ASSERT_FATAL_WITH_NODE(value, storeProfileValueSymRef != NULL, "ProfilingValue is neither stored in register nor in temp slot before split point.");
      }
   ...
   if (generateProfilingTreesInMainline)
      ...
      {
      // Trees are generated in the out-of-line section with the guard test to check if enableJProfiling flag is set or not.
      // For patchable JProfiling, guard node should be changed to a patchable dummy guard once we add runtime assumptions for them,
      ...
      if (mainlineReturnEntryGlRegDeps != NULL)
         ...
         glRegDepsToAttach = glRegDepsToAttach->duplicateTree(false);
