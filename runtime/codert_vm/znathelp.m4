...
BEGIN_FUNC($1)
    ST_GPR CRA,JIT_GPR_SAVE_SLOT(CRA)
    CALL_C($1, fast_$1)
    LHI_GPR r0,0
    CLR_GPR CRINT,r0
    je LABEL_NAME(CONCAT(L_DONE_,SYM_COUNT))
    L_GPR CRA,JIT_GPR_SAVE_SLOT(14)
    ST_GPR CRA,J9TR_VMThread_jitReturnAddress(J9VMTHREAD)
...
PLACE_LABEL(CONCAT(L_DONE_,SYM_COUNT))
    L_GPR CRA,JIT_GPR_SAVE_SLOT(CRA)
    L_GPR CRINT,J9TR_VMThread_returnValue(J9VMTHREAD)
    br CRA
END_CURRENT
})
